...
class mainWindow(QMainWindow):
    ...
    def __init__(self):
        ...
        self.OutputCombobox.currentIndexChanged.connect(self.apply_handler)
    ...
    def apply_handler(self):
        modified_data = self.apply_weights(self.OutputCombobox.currentText())

        if self.OutputCombobox.currentText() == 'FT Magnitude':
    ...
    def resize_images(self):
        ...
        for port in self.image_plots:
            if port.original_image_attr[RAW_IMG] is not None:
                height, width = port.original_image_attr[RAW_IMG].shape[:2]
